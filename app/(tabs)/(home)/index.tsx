...
export default function HomeScreen() {
  ...
  console.log('[Home] Rendering home screen', { user: user?.id, loading });
  ...
  useEffect(() => {
    const checkTeamsAndRedirect = async () => {
      ...
      console.log('[Home] Checking teams for post-login redirect');
      setCheckingTeams(true);

      try {
        // Fetch user's clubs
        const clubsData = await authenticatedGet<Club[]>('/api/clubs');
        console.log('[Home] Fetched clubs:', clubsData);

        if (!clubsData || clubsData.length === 0) {
          console.log('[Home] No clubs found, redirecting to get-started');
        ...
        for (const club of clubsData) {
          try {
            ...
            console.error(`[Home] Failed to fetch teams for club ${club.id}:`, error);
          }
        }

        console.log('[Home] Total teams found:', allTeams.length);

        if (allTeams.length === 0) {
          console.log('[Home] No teams found, redirecting to create team');
          router.replace({
        ...
        console.log('[Home] Last selected team ID:', lastTeamId);

        if (lastTeamId && allTeams.find((t) => t.id === lastTeamId)) {
          console.log('[Home] Auto-loading last selected team');
          router.replace({
        ...
        console.log('[Home] Multiple teams found, redirecting to select team');
        router.replace('/select-team');
      } catch (error) {
        console.error('[Home] Error checking teams:', error);
