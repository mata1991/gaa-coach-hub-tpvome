...
export default function TeamLineoutScreen() {
  ...
  // Get all selected player IDs across starting and bench for current side
  const selectedPlayerIds = useMemo(() => {
    if (!currentSquad) return new Set<string>();
    
    const ids = new Set<string>();
    currentSquad.startingSlots.forEach(slot => {
      if (slot.playerId) ids.add(slot.playerId);
    });
    currentSquad.bench.forEach(slot => {
      if (slot.playerId) ids.add(slot.playerId);
    });
    return ids;
  }, [currentSquad]);
