...
export function registerExportRoutes(app: App) {
  ...
  // GET /api/fixtures/:fixtureId/export/pitch - Export pitch view with lineup data
  app.fastify.get(
    '/api/fixtures/:fixtureId/export/pitch',
    async (request: FastifyRequest<{ Params: { fixtureId: string } }>, reply: FastifyReply) => {
      const session = await requireAuth(request, reply);
      if (!session) return;

      const { fixtureId } = request.params;
      app.logger.info({ userId: session.user.id, fixtureId }, 'Exporting pitch view');

      try {
        // Fetch fixture with squads and match state
        const fixture = await app.db.query.fixtures.findFirst({
        ...
        if (!fixture) {
          app.logger.warn({ fixtureId }, 'Fixture not found for pitch export');
          return reply.status(404).send({ error: 'Fixture not found' });
        }
        ...
        const pitchData = {
          ...
          fixture: {
            ...
            homeCrestUrl: fixture.homeCrestImageUrl,
            awayCrestUrl: fixture.awayCrestImageUrl,
          },
          ...
          squads: squads.map((squad) => ({
            side: squad.side,
            startingSlots: squad.startingSlots,
            bench: squad.bench,
            locked: squad.locked,
          })),
        ...
        app.logger.info({ fixtureId }, 'Pitch view exported');
        return pitchData;
      } catch (error) {
        app.logger.error({ err: error, fixtureId }, 'Failed to export pitch view');
        throw error;
      }
    }
  );

  // POST /api/fixtures/:fixtureId/export/image - Generate pitch image (stub)
  app.fastify.post(
    '/api/fixtures/:fixtureId/export/image',
    async (request: FastifyRequest<{ Params: { fixtureId: string } }>, reply: FastifyReply) => {
      const session = await requireAuth(request, reply);
      if (!session) return;

      const { fixtureId } = request.params;
      app.logger.info({ userId: session.user.id, fixtureId }, 'Generating pitch image');
      ...
      try {
        ...
        if (!fixture) {
          app.logger.warn({ fixtureId }, 'Fixture not found for image export');
          return reply.status(404).send({ error: 'Fixture not found' });
        }

        // Stub: Return a URL that would be generated by image rendering service
        const imageUrl = `/api/fixtures/${fixtureId}/pitch-image.png`;

        app.logger.info({ fixtureId, imageUrl }, 'Pitch image export generated');
        return {
          url: imageUrl,
          message: 'Pitch image generated successfully',
          note: 'In production, use a service like Puppeteer or Sharp to generate the actual image',
          contentType: 'image/png',
        };
      } catch (error) {
        app.logger.error({ err: error, fixtureId }, 'Failed to generate pitch image export');
        throw error;
      }
    }
  );
