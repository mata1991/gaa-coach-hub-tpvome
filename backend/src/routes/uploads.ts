...
export function registerUploadRoutes(app: App) {
  ...
  const uploadHandler = async (
    ...
    try {
      ...
      // Validate file type - only PNG, JPG, WebP (as specified in requirements)
      const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/webp'];
      if (!allowedMimeTypes.includes(data.mimetype)) {
        app.logger.warn({ userId: session.user.id, mimeType: data.mimetype }, 'Invalid file type');
        return reply.status(400).send({ error: 'Only PNG, JPG, and WebP images are allowed' });
      }
  ...
  app.fastify.post('/api/upload/image', uploadHandler);

  // POST /api/uploads - Upload an image file (alias for convenience)
  app.fastify.post('/api/uploads', uploadHandler);
}
